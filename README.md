...
├── sim/           Simulation gates that must pass before fab — strictly
│                  the laptop's physical/electrical behavior:
│                    thermal (OpenFOAM), mechanical (Fusion/FreeCAD FEM),
│                    electrical (ngspice), signal-integrity (OpenEMS),
│                    firmware (QEMU + Wokwi)
└── website/       Marketing + docs site (Astro)
```

> **Simulate first, fab second.** mang0 is ~$5.6K of parts with multi-week chassis and vapor-chamber lead times. The [`sim/`](./sim/) directory holds gates every subsystem must pass before fabrication is allowed.
